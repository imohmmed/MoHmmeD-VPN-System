...
export default function SubOwnerAgentsPage() {
  ...
  const { data: agents = [], isLoading } = useQuery<Agent[]>({ queryKey: ["/api/my-agents"] });
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiRequest("POST", "/api/my-agents", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/my-agents"] });
  ...
  const suspendMutation = useMutation({
    mutationFn: (id: string) => apiRequest("PATCH", `/api/my-agents/${id}/suspend`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/my-agents"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/my-agents/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/my-agents"] });
      queryClient.invalidateQueries({ queryKey: ["/api/stats"] });
      setDeleteId(null);
  ...
  const paymentMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) =>
      apiRequest("POST", `/api/my-agents/${id}/payment`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/my-agents"] });
      queryClient.invalidateQueries({ queryKey: ["/api/stats"] });
      ...
      <div className="space-y-5 max-w-5xl">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-0.5">Manage your reseller accounts</p>
          </div>
                ...
                <form onSubmit={form.handleSubmit((d) => createMutation.mutate(d))} className="space-y-4">
                  ...
                  <FormField control={form.control} name="prefix" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input {...field} placeholder="e.g. star, moon, vpn1" data-testid="input-agent-prefix" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
          ...
          <div className="space-y-3">
            {filtered.map((agent) => (
              <Card
                ...
                onClick={() => setLocation(`/sub-owner/agents/${agent.id}`)}
              >
                    ...
                    <div className="flex items-center gap-1.5">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => setLocation(`/sub-owner/agents/${agent.id}`)} data-testid={`button-view-${agent.id}`}>
